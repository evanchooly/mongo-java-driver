/*
 * Copyright (c) 2008-2016 MongoDB, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.bson.codecs.configuration.mapper.entities;

import org.bson.types.ObjectId;

import java.util.Collection;
import java.util.LinkedList;
import java.util.List;
import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;

public class Concrete {
    private ObjectId id;
    private Long alternateId;
    private LinkedList<Integer> linked;
    private ConcurrentHashMap<String, Double> concurrent;
    private List<Integer> list;
    private Collection<Integer> collection;
    private Map<String, Double> map;

    public Long getAlternateId() {
        return alternateId;
    }

    public void setAlternateId(final Long alternateId) {
        this.alternateId = alternateId;
    }

    public Collection<Integer> getCollection() {
        return collection;
    }

    public void setCollection(final Collection<Integer> collection) {
        this.collection = collection;
    }

    public ConcurrentHashMap<String, Double> getConcurrent() {
        return concurrent;
    }

    public void setConcurrent(final ConcurrentHashMap<String, Double> concurrent) {
        this.concurrent = concurrent;
    }

    public ObjectId getId() {
        return id;
    }

    public void setId(final ObjectId id) {
        this.id = id;
    }

    public LinkedList<Integer> getLinked() {
        return linked;
    }

    public void setLinked(final LinkedList<Integer> linked) {
        this.linked = linked;
    }

    public List<Integer> getList() {
        return list;
    }

    public void setList(final List<Integer> list) {
        this.list = list;
    }

    public Map<String, Double> getMap() {
        return map;
    }

    public void setMap(final Map<String, Double> map) {
        this.map = map;
    }

    @Override
    public int hashCode() {
        int result = getId() != null ? getId().hashCode() : 0;
        result = 31 * result + (getAlternateId() != null ? getAlternateId().hashCode() : 0);
        result = 31 * result + (getLinked() != null ? getLinked().hashCode() : 0);
        result = 31 * result + (getConcurrent() != null ? getConcurrent().hashCode() : 0);
        result = 31 * result + (getList() != null ? getList().hashCode() : 0);
        result = 31 * result + (getCollection() != null ? getCollection().hashCode() : 0);
        result = 31 * result + (getMap() != null ? getMap().hashCode() : 0);
        return result;
    }

    @Override
    public boolean equals(final Object o) {
        if (this == o) {
            return true;
        }
        if (!(o instanceof Concrete)) {
            return false;
        }

        Concrete concrete = (Concrete) o;

        if (getId() != null ? !getId().equals(concrete.getId()) : concrete.getId() != null) {
            return false;
        }
        if (getAlternateId() != null ? !getAlternateId().equals(concrete.getAlternateId()) : concrete.getAlternateId() != null) {
            return false;
        }
        if (getLinked() != null ? !getLinked().equals(concrete.getLinked()) : concrete.getLinked() != null) {
            return false;
        }
        if (getConcurrent() != null ? !getConcurrent().equals(concrete.getConcurrent()) : concrete.getConcurrent() != null) {
            return false;
        }
        if (getList() != null ? !getList().equals(concrete.getList()) : concrete.getList() != null) {
            return false;
        }
        if (getCollection() != null ? !getCollection().equals(concrete.getCollection()) : concrete.getCollection() != null) {
            return false;
        }
        return getMap() != null ? getMap().equals(concrete.getMap()) : concrete.getMap() == null;

    }
}
